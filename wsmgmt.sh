...
function __create_label_from_path {
	...
	do
		if [[ ${array[index]} == ${WS_SDK##*/} ]]; then					# if the string at position of index of array is the last folder in the WS_SDK path
...
function ws {
	if [ -z "${1}" ]; then					# if no specific workspace is given,
